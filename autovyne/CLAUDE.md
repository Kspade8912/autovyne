...
- `leads` — Prospect leads stored via `/api/leads`: biz name, industry, call volume, miss rate, estimated missed leads/loss

## External integrations
- Email: nodemailer via SMTP (env vars: SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, SMTP_FROM, LEAD_NOTIFY_EMAIL)
...
- 2026-05-22: Security hardening (initial): Helmet-style headers (CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy), CORS locked to autovyne.com + localhost, in-memory rate limiting (100 req/15min global, 5 POSTs/15min on /api/leads), XSS sanitization on all form inputs, honeypot + 3s min-submit on /api/leads. Generic error handler. New `lib/security.js`.
